...
/* At the top of spray_inject_custom.c, before any function definitions: */
static long spray_inject_custom_call_count = 0;
...

{
	...
	spray_inject_custom_call_count++;

	/* (Option A) Print every N calls—for example every 10,000 injections: */
	if (spray_inject_custom_call_count % 1000 == 0 && rank == 0) {
		printf("[spray_inject_custom] called %ld times so far\n",
		       spray_inject_custom_call_count);
	}

    {
	    ...
	    *passed_zdist = passed_random_fill * rad_local * (norm_vec[2] * cos(angle_fill) + other_vec[2] * sin(angle_fill));
	    ...
	    printf ("spray_inject_custom.c fuel_den = %g pvap_liq = %g  hvap_liq = %g fuel_temp = %g\n", fuel_den, fuel_pvap, fuel_hvap, fuel_temp);
	    printf ("spray_inject_custom.c sigma = %g\n", sigma);
		printf ("spray_inject_custom.c fuel cond = %g\n", fuel_cond);
	    ...
	    CONVERGE_precision_t drop_radius_fact, g, vap_den;
	    CONVERGE_precision_t noz_dia = CONVERGE_nozzle_get_parameter_precision(nozzle, NOZZLE_DIAMETER);
	    CONVERGE_precision_t noz_len = CONVERGE_nozzle_get_parameter_precision(nozzle, NOZZLE_LENGTH);
	    CONVERGE_precision_t m_total = CONVERGE_injector_get_parameter_precision(injector, INJECTOR_INJECT_MASS);
	    CONVERGE_precision_t duration = CONVERGE_injector_get_parameter_precision(injector, INJECTOR_INJECT_DURATION);
	    CONVERGE_precision_t mdot = m_total / duration;


	   // printf("Nozzle diameter: %g\n", noz_dia);
	   // printf("Nozzle length: %g\n", noz_len);

	    theta_c = 45.78; //bubble surface contact angle
	    y = 4.4; //eqn 2895 exponent
	    C_c0 = 0.611; //baseline contraction coefficient
	    r_D = 0.07; //roundness ratio for rounded inlet nozzle
	    g = 9.81;
	    T_s = 349.1; //T_sat of fuel at ambient pressure conditions (need to modify this depending on the case)
	    vap_den = ambient_density; //eqn 2890
	    ...
	    // S_noz = noz_dia * noz_len * M_PI; //inner lateral surface area of nozzle
	    S_noz = 0.25 * M_PI * pow(noz_dia,2); //cross-sectional area of nozzle
	    ...
	    vdot_tot = v_vena * S_noz; //calculate total vol flowrate in nozzle
	    drop_radius_fact = (vdot_tot-vdot_vap)/vdot_tot;
	    printf("vdot_vap = %.17g, vdot_tot = %.17g\n", vdot_vap, vdot_tot);
	    printf("D_b = %g, rho_star = %g, bub_freq = %g, Nstar_nuc = %g, F_rhostar = %g\n", D_b, rho_star, bub_freq, Nstar_nuc, F_rhostar);
	    printf("V_b = %g, S_noz = %g, N_nuc = %g\n", V_b, S_noz, N_nuc);
